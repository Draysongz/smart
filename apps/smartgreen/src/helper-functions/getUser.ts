...
async function updateReferralData(userId: number, referralId: number) {
  try {
    ...
    const referralQuery = await getQuerySnapshot(referralId);
    ...
    const userQuery = await getQuerySnapshot(userId);
    if (userQuery.empty) {
      console.log("No user found with that ID.");
      return null; // Or throw an error if preferred
    }

    const referralDocRef = doc(db, "smart", referralQuery.docs[0].id);
    const userDocRef = doc(db, "smart", userQuery.docs[0].id);

    // Check if the user is already in the referrer's referrals array
    const referralDoc = referralQuery.docs[0].data();
    if (referralDoc.referrals.includes(userId)) {
